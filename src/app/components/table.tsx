...
import { LaporanFormat } from "../dashboard/utils/reportUtils";
...
export default function Table({ data, itemsPerPage = 8, filterStatus }: Props) {
  ...
  const filteredData = data
    ...
    .filter((item) => item.Username.toLowerCase().includes(searchTerm.toLowerCase()) || item.Title.toLowerCase().includes(searchTerm.toLowerCase()) || item.Description.toLowerCase().includes(searchTerm.toLowerCase()));
        ...
        <tbody className="text-black font-medium font-satoshi">
          {currentItems.length > 0 ? (
            currentItems.map((item) => (
              <tr key={item.Id} onClick={() => router.push(`/dashboard/all/${item.Id}`)} className="border-b border-[#E0E0E0] cursor-pointer hover:bg-gray-50">
